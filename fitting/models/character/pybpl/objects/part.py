...
from .. import splines
from tools.geometry import get_2D_rotation_matrix
import numpy as np
...
def vanilla_to_motor_with_rotation(shapes, invscales, first_pos, angles, neval=200):
    """
    Create the fine-motor trajectory of a stroke (denoted 'f()' in pseudocode)
    with 'nsub' sub-strokes.
    Reference: BPL/classes/Stroke.m (lines 203-238)
    ...
    :param invscales: [(nsub,) tensor] inverse scales for each sub-stroke
    :param first_pos: [(2,) tensor] starting location of stroke
    :param neval: [int] number of evaluations to use for each motor trajectory
    :return:
        motor: [(nsub,neval,2) tensor] fine motor sequence
        motor_spline: [(ncpt,2,nsub) tensor] fine motor sequence in spline space
    """
    ...
    for i in range(nsub):
        ...
        offset = traj[0] - previous_pos
        motor[i] = traj - offset
        rotation = get_2D_rotation_matrix(angles[i], motor[i, 0])
        rotation = np.asarray(rotation)
        motor_numpy = motor[i].numpy()
        aug = np.ones((motor_numpy.shape[0]))
        motor_aug = np.vstack((motor_numpy.transpose(), aug))
        motor[i] = torch.tensor((rotation @ motor_aug).transpose())
