...
class CharacterImageDist(ConceptImageDist):
    ...
    def get_image(self, motors, scales):
        affine = torch.zeros(4)
        affine[0:2] = torch.from_numpy(scales)
        # affine[2], affine[3] = state.position_x, state.position_y
        motors = apply_warp(motors, affine)
        motors_flat = torch.cat(motors)
        pimg, _ = render_image(
            motors_flat, self.ps.min_epsilon, self.ps.min_blur_sigma, self.ps)
