...
from models.road_curve.curve_rule import CurveRule as Rule

from tools.data_tool import load_road_data as load_data
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='configs/fit_road.yaml')
    ...
    # 提取文件名和上一级文件夹名，构建清晰的输出日志目录
    data_name = data_file.split('/')[-1].split('.')[0]
    folder_name = data_file.split('/')[-2] if len(data_file.split('/')) > 1 else "default"
    base_cfg['record']['root_dir'] = f"./outputs/{algo}/road/{folder_name}/{data_name}/run_{run_id}/"
    ...
    print("=" * 70)
    print(f"Task: 3D ROAD CURVE | Algorithm: {algo.upper()} | Estimator: {est_type.upper()}")
