...
class Fitter:
    def __init__(self, cfg):
        ...
        self.coverage_weight = float(fitter_cfg.get("gd_coverage_weight", 0.5))
        self.coverage_threshold_factor = float(
            fitter_cfg.get("gd_coverage_threshold_factor", 2.5)
        )
        self.coverage_temperature_factor = float(
            fitter_cfg.get("gd_coverage_temperature_factor", 0.5)
        )
        ...
        self.overlap_weight = float(fitter_cfg.get("gd_overlap_weight", 0.05))
    ...
    def optimize_instance(self):
        ...
        weights_raw = torch.nn.Parameter(torch.logit(((init_weights - self.weight_lb) / (self.weight_ub - self.weight_lb)).clamp(1e-4, 1 - 1e-4)))

        optimizer = torch.optim.Adam([control_points, weights_raw], lr=self.lr)
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer,
            T_max=max(self.max_steps, 1),
        ...
        coverage_temperature = max(
            self.coverage_temperature_factor * self.data_resolution,
        ...
        for step in range(1, self.max_steps + 1):
            ...
            data_to_model_min = pairwise.min(dim=1).values
            model_to_data_min = pairwise.min(dim=0).values
            data_to_model = data_to_model_min.mean()
            model_to_data = model_to_data_min.mean()
            soft_coverage = torch.sigmoid(
                (coverage_threshold - data_to_model_min) / coverage_temperature
            ).mean()

            second_u = control[2:, :, :] - 2.0 * control[1:-1, :, :] + control[:-2, :, :]
            second_v = control[:, 2:, :] - 2.0 * control[:, 1:-1, :] + control[:, :-2, :]
            smoothness = second_u.norm(dim=-1).mean() + second_v.norm(dim=-1).mean()
            smoothness = smoothness / max(
                float(torch.linalg.norm(self.data_max - self.data_min).item()),
                self.data_resolution,
                np.finfo(np.float32).eps,
            )

            below = torch.relu(self.data_min - model_points)
            above = torch.relu(model_points - self.data_max)
            bbox_penalty = (below + above).sum(dim=1).mean()
            ...
            if base_cloud is not None and base_cloud.numel() > 0:
                ...
                margin = 2.0 * self.data_resolution
                overlap_penalty = torch.relu(margin - overlap_dist.min(dim=1).values).mean()

            loss = (
                self.data_to_model_weight * data_to_model
            ...
            if step % self.eval_interval == 0 or step == 1 or step == self.max_steps:
                with torch.no_grad():
                    ...
                    weights_eval = (
                        self.weight_lb
                        + (self.weight_ub - self.weight_lb) * torch.sigmoid(weights_raw)
                    ).detach().cpu().numpy()
                ...
                print(
                    f"GD Step: {step}/{self.max_steps}, "
                    f"Loss: {loss.item():.6f}, Score: {score:.6f}",
                    end="\r",
                    flush=True,
                )
