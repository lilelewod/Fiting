...
class MeanMeasureEstimator:
    def __init__(self, cfg):
        ...
        # ===== 新增 WMM 核心参数 =====
        self.h = cfg['estimator'].get('h', 0.0)  # h=0 时退化为普通 MM，h>0 时为加权 WMM
        self.epsilon = 1e-8  # 防止除零的极小值

        self.weighted_measure = 0.0
        self.weighted_sum_errors = 0.0
        self.initial_weighted_measure = 0.0
        self.initial_weighted_sum_errors = 0.0
        # =============================
    ...
    def reset(self):
        ...
        self.model_color = deepcopy(self.initial_model_color)
        self.measure = 0

        # WMM 状态重置
        self.weighted_measure = deepcopy(self.initial_weighted_measure)
        self.weighted_sum_errors = deepcopy(self.initial_weighted_sum_errors)
    ...
    def update(self, model, sum_errors, nearest_points, labels, instance_index):
        ...
        # 巧妙地在此处重新计算权重视图，无需改变与 collector 的通信协议
        if model.shape[0] > 0 and self.data_kDTree is not None:
            errors, _ = self.data_kDTree.query(model)
            raw_errors = errors.flatten()
            if self.h > 0:
                weights = np.exp(-self.h * raw_errors)
                self.initial_weighted_measure = np.sum(weights)
                self.initial_weighted_sum_errors = np.sum(weights * raw_errors)
            else:
                self.initial_weighted_measure = float(model.shape[0])
                self.initial_weighted_sum_errors = np.sum(raw_errors)
        else:
            self.initial_weighted_measure = 0.0
            self.initial_weighted_sum_errors = 0.0
    ...
    def mean_measure(self):
        ...
        if self.weighted_measure <= 0:
            return -1

        # 1. 计算原始的加权平均误差
        error = self.weighted_sum_errors / self.weighted_measure

        # ================= 新增：归一化处理 =================
        # 将测度(点数)归一化到 0~1 左右：除以目标图片的总有效点数
        normalized_measure = self.weighted_measure / float(self.num_data_points)
        ...
        # 2. 计算归一化后的最终得分
        score = (normalized_measure ** self.regularization_factor) / (normalized_error + self.epsilon)
        return score

    def compute_model_to_data_error(self, model):
        # 改进：现在同时返回总误差和每个点的原始误差数组
        if self.data_kDTree is None:
            ...
            return sum_errors, errors.flatten()
    ...
    def add_model(self, **kwargs):
        ...
        if new_model.shape[0] < 5:
            if self.current_dividing_level != 0:
        ...
        # 计算误差，并分离出每个点的距离
        sum_errors, raw_errors = self.compute_model_to_data_error(new_model)
        self.single_model_error = sum_errors / float(new_model.shape[0])

        if raw_errors is not None:
            if self.h > 0:
                # 触发 WMM 逻辑：计算每个点的距离衰减权重
                weights = np.exp(-self.h * raw_errors)
                self.weighted_measure += np.sum(weights)
                self.weighted_sum_errors += np.sum(weights * raw_errors)
            else:
                # 退化为普通 MM 逻辑
                self.weighted_measure += kwargs.get('new_measure', new_model.shape[0])
                self.weighted_sum_errors += sum_errors
