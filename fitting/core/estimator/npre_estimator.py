...
class NPREEstimator:
    ...
    def __init__(self, cfg):
        ...
        self.early_rejection_enabled = cfg['estimator'].get('early_rejection', False) 
    ...
    def load_data(self):
        ...
        if self.data_resolution is None:
            ...
            # 如果外部加载器（如字符/直线任务）已经调用过 preprocess，我们只刷新一次 KDTree
            self.create_kdtree(data)
    ...
    def add_model(self, **kwargs):
        ...
        if new_model.shape[0] < 5:
           if self.early_rejection_enabled and self.current_dividing_level != 0:
            self.score, self.single_model_error = 1e-6 * new_model.shape[0], float('inf') # self.score, self.single_model_error = -1 , float('inf')
            return
